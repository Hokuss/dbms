...
bool dbms::irow(std::string table_name, std::vector<db_value> row_data) {
    ...
    if(layout.unique_key.find("none")==std::string::npos){
        int i = 0;
        while(i<layout.columns.size()){
            if(layout.columns[i].name == layout.unique_key) break;
            i++;
        }
        if(i>=layout.columns.size()) {
            std::cerr<<"[DBMS Error] Unique Key not found.\n";
            return false;
        }

        std::pair<std::string, db_value> uniques_check = {layout.unique_key, row_data[i]};
        std::vector<db_value> currently = grow(table_name,uniques_check);
        if(!currently.empty()){
            std::cerr<<"[DBMS Error] Already Exists\n";
            return false;
        }
    }

    if (row_data.size()!=layout.columns.size()){
        std::cerr<<"[DBMS Error] Schema mismatch! \n";
        return false;
    }
    ...
    try {
        ...
        std::cerr<<"[DBMS Error] Variant Type is not correct. \n";
        return false;
    }
    dat_file.close();
    blob_file.close();
...
bool dbms::load_schema(const std::string& table_name){
    ...
    while(std::getline(schema_file, line)) {
        if(line.find("create_time:")!=std::string::npos){
            ...
            new_schema.create_time = std::stoi(line.substr(12));
        } else if (line.find("unique_key:")!=std::string::npos){
            new_schema.unique_key = line.substr(11);
...
std::future<std::vector<std::vector<db_value>>> dbms::get_rows(std::string table_name, std::pair<std::string, db_value> request) {
    return submit_task([this, table_name, request]() -> std::vector<std::vector<db_value>> {
        return this->grows(table_name, request); 
    });
}

std::vector<std::vector<db_value>> dbms::grows(std::string table_name, std::pair<std::string, db_value> request) {
    if (!load_schema(table_name)) return {};
    const table_schema& layout = schema_cache[table_name];
    ...
    std::vector<std::vector<db_value>> results; 
